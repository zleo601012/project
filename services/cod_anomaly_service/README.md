...
- **窗口长度**：`12` 条记录
- **适用场景**：用于检测 COD 指标的异常变化，并结合流量、降雨和温度背景降低误报。
...
对应训练入口位于：`training/cod_anomaly/train.py`

训练命令示例：

```bash
python training/cod_anomaly/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.cod_anomaly_service.app:app --host 0.0.0.0 --port 8103
```

启动后可访问：

- `http://127.0.0.1:8103/health`
- `http://127.0.0.1:8103/meta`
- `http://127.0.0.1:8103/infer`
...
```bash
docker build -f services/cod_anomaly_service/Dockerfile -t edge-offload/cod_anomaly_service:local .
docker run --rm -p 8000:8000 edge-offload/cod_anomaly_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/cod_anomaly_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
