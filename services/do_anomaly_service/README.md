...
- **窗口长度**：`12` 条记录
- **适用场景**：用于检测溶解氧指标异常，可帮助发现曝气失衡、传感异常或水体受扰动情况。
...
对应训练入口位于：`training/do_anomaly/train.py`

训练命令示例：

```bash
python training/do_anomaly/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.do_anomaly_service.app:app --host 0.0.0.0 --port 8106
```

启动后可访问：

- `http://127.0.0.1:8106/health`
- `http://127.0.0.1:8106/meta`
- `http://127.0.0.1:8106/infer`
...
```bash
docker build -f services/do_anomaly_service/Dockerfile -t edge-offload/do_anomaly_service:local .
docker run --rm -p 8000:8000 edge-offload/do_anomaly_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/do_anomaly_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
