...
- **窗口长度**：`24` 条记录
- **适用场景**：用于预测悬浮物或浊度在短时窗口内的变化，适合雨季冲击场景。
...
对应训练入口位于：`training/tss_turbidity_forecast/train.py`

训练命令示例：

```bash
python training/tss_turbidity_forecast/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.tss_turbidity_forecast_service.app:app --host 0.0.0.0 --port 8204
```

启动后可访问：

- `http://127.0.0.1:8204/health`
- `http://127.0.0.1:8204/meta`
- `http://127.0.0.1:8204/infer`
...
```bash
docker build -f services/tss_turbidity_forecast_service/Dockerfile -t edge-offload/tss_turbidity_forecast_service:local .
docker run --rm -p 8000:8000 edge-offload/tss_turbidity_forecast_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/tss_turbidity_forecast_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
