...
- **窗口长度**：`12` 条记录
- **适用场景**：用于识别悬浮物和浊度的异常变化，适合降雨扰动与泥沙冲击相关场景。
...
对应训练入口位于：`training/tss_turbidity_anomaly/train.py`

训练命令示例：

```bash
python training/tss_turbidity_anomaly/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.tss_turbidity_anomaly_service.app:app --host 0.0.0.0 --port 8105
```

启动后可访问：

- `http://127.0.0.1:8105/health`
- `http://127.0.0.1:8105/meta`
- `http://127.0.0.1:8105/infer`
...
```bash
docker build -f services/tss_turbidity_anomaly_service/Dockerfile -t edge-offload/tss_turbidity_anomaly_service:local .
docker run --rm -p 8000:8000 edge-offload/tss_turbidity_anomaly_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/tss_turbidity_anomaly_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
