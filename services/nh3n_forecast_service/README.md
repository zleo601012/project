...
- **窗口长度**：`24` 条记录
- **适用场景**：用于预测 NH3-N 的短时变化，可作为氨氮趋势预警的输入服务。
...
对应训练入口位于：`training/nh3n_forecast/train.py`

训练命令示例：

```bash
python training/nh3n_forecast/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.nh3n_forecast_service.app:app --host 0.0.0.0 --port 8203
```

启动后可访问：

- `http://127.0.0.1:8203/health`
- `http://127.0.0.1:8203/meta`
- `http://127.0.0.1:8203/infer`
...
```bash
docker build -f services/nh3n_forecast_service/Dockerfile -t edge-offload/nh3n_forecast_service:local .
docker run --rm -p 8000:8000 edge-offload/nh3n_forecast_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/nh3n_forecast_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
