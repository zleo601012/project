...
- **窗口长度**：`12` 条记录
- **适用场景**：用于输出雨污混接嫌疑风险分值，便于在边缘设备侧快速打分和排序。
...
对应训练入口位于：`training/mixed_sewage_rain_score/train.py`

训练命令示例：

```bash
python training/mixed_sewage_rain_score/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.mixed_sewage_rain_score_service.app:app --host 0.0.0.0 --port 8301
```

启动后可访问：

- `http://127.0.0.1:8301/health`
- `http://127.0.0.1:8301/meta`
- `http://127.0.0.1:8301/infer`
...
```bash
docker build -f services/mixed_sewage_rain_score_service/Dockerfile -t edge-offload/mixed_sewage_rain_score_service:local .
docker run --rm -p 8000:8000 edge-offload/mixed_sewage_rain_score_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/mixed_sewage_rain_score_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
