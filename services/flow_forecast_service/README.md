...
`flow_forecast_service` 现在是一个**自包含的独立流量预测服务**：
...
```bash
python3 -m services.flow_forecast_service.train --dataset dataset/node_1.csv
```

### 方式 2：启动 HTTP 服务

```bash
python3 -m services.flow_forecast_service.server
```
...
- `services/flow_forecast_service/artifacts/flow_forecast_service.joblib`
- `services/flow_forecast_service/artifacts/flow_forecast_service.metadata.json`

也可以通过环境变量覆盖：

```bash
MODEL_DIR=/data/models python3 -m services.flow_forecast_service.server
```
...
```bash
docker run --rm -p 8201:8000 -e MODEL_DIR=/app/services/flow_forecast_service/artifacts edge-offload/flow-forecast-service:standalone
```

如果需要在容器里训练，可以挂载数据集后调用 `/train`，或者在容器里执行：

```bash
python -m services.flow_forecast_service.train --dataset /data/node_1.csv --output-dir /app/services/flow_forecast_service/artifacts
```
## 1. 服务定位
...
对应训练入口位于：`training/flow_forecast/train.py`
...
```bash
docker build -f services/flow_forecast_service/Dockerfile -t edge-offload/flow_forecast_service:local .
docker run --rm -p 8000:8000 edge-offload/flow_forecast_service:local
```
