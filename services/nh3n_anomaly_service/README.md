...
- **窗口长度**：`12` 条记录
- **适用场景**：用于检测氨氮指标的异常波动，可作为污染冲击或异常排放的辅助识别服务。
...
对应训练入口位于：`training/nh3n_anomaly/train.py`

训练命令示例：

```bash
python training/nh3n_anomaly/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.nh3n_anomaly_service.app:app --host 0.0.0.0 --port 8104
```

启动后可访问：

- `http://127.0.0.1:8104/health`
- `http://127.0.0.1:8104/meta`
- `http://127.0.0.1:8104/infer`
...
```bash
docker build -f services/nh3n_anomaly_service/Dockerfile -t edge-offload/nh3n_anomaly_service:local .
docker run --rm -p 8000:8000 edge-offload/nh3n_anomaly_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/nh3n_anomaly_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
