...
- **窗口长度**：`12` 条记录
- **适用场景**：用于综合判断水质指标的联合异常，适合在多指标场景下做异常筛查，而不是单指标阈值报警。
...
对应训练入口位于：`training/water_quality_anomaly/train.py`

训练命令示例：

```bash
python training/water_quality_anomaly/train.py --dataset dataset/node_1.csv
```
...
```bash
uvicorn services.water_quality_anomaly_service.app:app --host 0.0.0.0 --port 8102
```

启动后可访问：

- `http://127.0.0.1:8102/health`
- `http://127.0.0.1:8102/meta`
- `http://127.0.0.1:8102/infer`
...
```bash
docker build -f services/water_quality_anomaly_service/Dockerfile -t edge-offload/water_quality_anomaly_service:local .
docker run --rm -p 8000:8000 edge-offload/water_quality_anomaly_service:local
```

> 如果需要在容器内执行推理，请先在宿主机生成 `models/trained/water_quality_anomaly_service.joblib`，或在镜像构建前将训练产物放入构建上下文。
